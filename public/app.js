...
let backendUrl =
  location.protocol === "file:" ? "https://tiktok-chat-reader.zerody.one/" : undefined;
...
function updateRoomStats() {
  $("#roomStats")
    .html(`Viewers: <b>${viewerCount.toLocaleString()}</b> Likes: <b>${likeCount.toLocaleString()}</b> 
...
function isPendingStreak(data) {
  return data.giftType === 1 && data.repeatEnd === 0;
}
                ...
                <div>
                    <table>
                        <tr>
                            ...
                            <td>
                                <span>Name: <b>${data.giftDetails.giftName}</b> (ID:${
    data.giftId
  })<span><br>
                                <span>Repeat: <b style="${
                                ...
                                <span>Cost: <b>${(
                                  data.giftDetails.diamondCount * data.repeatCount
...
connection.on("gift", (data) => {
  ...
  if (data.giftDetails.diamondCount > 0) {
    // Add diamonds total
    const totalDiamonds = data.giftDetails.diamondCount * data.repeatCount;
